...
app.post('/UPDATE_FINAL', async (req, res) => {
  const { id, title, topic, price, quantity } = req.body; // Destructuring directly from req.body

  if (!id) {
    return res.status(400).send('ID is required.');
  }

  const newData = { title, topic, price, quantity };
  
  try {
    updateCatalogEntry(id, newData); // Call the update function
    res.status(200).send(`Book with ID ${id} updated successfully.`);
  } catch (error) {
    console.error(error);
    res.status(500).send('An error occurred while updating the catalog entry.');
  }
});
